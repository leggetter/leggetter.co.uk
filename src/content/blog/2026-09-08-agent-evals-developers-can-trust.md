...
Ours has web search too, and our published baseline doesn't say so. Agents used it on 34 of the 114 rows in the last snapshot. Say it if yours has search, because a baseline that can search is partly measuring how well other people's blog posts cover your product, and that changes between runs without you touching anything. [Convex](https://www.convex.dev/llm-leaderboard) handles this properly: its leaderboard has three tabs, and web access is a configuration of its own instead of something folded quietly into the baseline.
