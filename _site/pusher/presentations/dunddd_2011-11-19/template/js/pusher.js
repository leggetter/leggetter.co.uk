...
// Pusher defaults
Pusher.VERSION = '1.9.3-phil_improved_auth';
  ...
  ajax: function(pusher, callback){
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState == 4) {
        ...
        self.dispatch('pusher:after_auth', evt);
        ...
        if (xhr.status == 200) {
          try {
            ...
            throw "Could not parse authtication response from your auth endpoint. Received: \"" + evt.responseText + "\"";
          }
        } else {
    ...
    setTimeout(function() {
      var evt = {
        url: Pusher.channel_auth_endpoint,
        data: data,
        xhr: xhr
      };
      self.dispatch('pusher:before_auth', evt);
    
      var sendData = "";
      for(var paramName in evt.data){
        sendData += encodeURIComponent(paramName) + "=" + encodeURIComponent(evt.data[paramName]) + "&";
      }
