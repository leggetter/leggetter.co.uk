...
| `src/games/deadball/` | The dead ball game - penalties now, free kicks later - served from the hidden page at `/deadball/`. **Separately licensed: BSD-3-Clause, see `src/games/deadball/LICENSE`.** The rest of the repo is CC-BY-3.0, which Creative Commons themselves advise against using for software. `npm run test:game` and `npm run typecheck`. Design notes in `docs/deadball-spec.md`; the doc to hand a new contributor is `docs/deadball-jobs.md`. |
